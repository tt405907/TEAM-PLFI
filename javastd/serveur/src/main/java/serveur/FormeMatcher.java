...
//				+ m44 + " iHu1 : " + iHu1 + "iHu2 : " + iHu2 + " iHu3 : " + iHu3 + " iHu4 : " + iHu4);
		 ...
		 */
		if (circleTester(points, barycentre, 0.1))
			return Forme.ROND;
		return Forme.TRIANGLE;
	}
	...
	private boolean circleTester(List<Point> points, Point barycentre, double errorMarge) {
		float db = distancePoint(barycentre, points.get(0));
		int errorCount = 0;
		for (Point p : points) {
			float d = distancePoint(barycentre, p);
			if (d < (1 - errorMarge) * db && d > (1 + errorMarge) * db)
				errorCount += 1;
		}
		if (errorCount > errorMarge * points.size())
			return false;
		return true;
	}
